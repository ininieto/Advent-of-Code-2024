...
// Function that will return the lowest score for the current grid. Will return -1 if no solution
int getLowestScore(const std::vector<std::vector<char>>& grid) {
...
int main() {
    ...
    while (std::getline(lineStream, line)) {
        ...
        if (std::getline(pairStream, aStr, ',') && std::getline(pairStream, bStr)) {
            ...
            // Start the algorithm after Part 1
            if (byteCount <= minNumBytes)
                continue;

            // Find the minimum distance for this configuration
            int lowestScore = getLowestScore(grid);

            if (lowestScore < 0) {
                std::cout << "The solution is " << a << "," << b << '\n';
                break;
            }
        }
    }
