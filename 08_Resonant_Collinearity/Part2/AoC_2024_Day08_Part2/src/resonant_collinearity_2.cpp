...
#include "resonant_collinearity_2.h"
	...
	for (const auto& freq : freqPositions) {
		...
		for (int i = 0; i < posVector.size(); i++) {
			...
			for (int j = 0; j < posVector.size(); j++) {
				...
				// Get all the possible antinodes
				Coords antinode1;
				Coords antinode2;

				// Create a copy of the vector
				Coords vectorCopy = vector;

				// Antinode 1
				while (!outOfBounds(antinode1 = Coords(currentPosition.i - vectorCopy.i, currentPosition.j - vectorCopy.j), nrows, ncols)) {
					grid[antinode1.i][antinode1.j].isAntinode = true;
					vectorCopy.i -= vector.i;
					vectorCopy.j -= vector.j;
				}

				// Reset the vector copy
				vectorCopy = vector;

				// Antinode 2
				while (!outOfBounds(antinode2 = Coords(currentPosition.i + vectorCopy.i, currentPosition.j + vectorCopy.j), nrows, ncols)) {
