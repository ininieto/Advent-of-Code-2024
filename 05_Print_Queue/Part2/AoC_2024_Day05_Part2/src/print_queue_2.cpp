...
int main() {
	...
	while (std::getline(inputStream, line)) {
		...
		if (readOrderingRules)
			...
			updates.push_back(Update(parseUpdates(line)));
	}
	...
	for (auto& update : updates) {
		...
		for (int i = 0; i < update.vec.size(); i++) {

			int currentNum = update.vec[i];

			// Traverse the rest of the vector trying to find a misplace
			for (int j = i + 1; j < update.vec.size(); j++) {

				int scannedNum = update.vec[j];
				...
				if (it != priority.end()) {
					...
					if (std::find(lowerPriority.begin(), lowerPriority.end(), currentNum) != lowerPriority.end()) {
						...
						// Swap the current and scanned numbers
						int temp = update.vec[i];
						update.vec[i] = update.vec[j];
						update.vec[j] = temp;
	...
	for (const auto& update : updates) {
		if(update.reordered)
			solution += update.vec[update.vec.size() / 2];
	}
